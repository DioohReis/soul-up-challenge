...
export function animateNexo(parts: NexoParts, frame: AnimationFrame) {
  ...
  const celebrationBounce = state === 'celebrating' ? Math.pow(Math.max(0, Math.sin(time * 5.4)), 2) * 0.085 * motion : 0
  parts.root.position.y = smooth(parts.root.position.y, parts.root.userData.baseY + celebrationBounce, 9, delta)
  parts.body.scale.y = smooth(parts.body.scale.y,
    parts.body.userData.baseScaleY * (1 + Math.sin(time * 1.7) * 0.0045 * motion), 5, delta)

  const followStrength = state === 'sleeping' ? 0.12 : state === 'thinking' ? 0.30 : 0.85
  const headNod = state === 'talking' ? Math.sin(time * 3.5) * 0.028 * motion : 0
  animateRotation(parts.head, 'y', pointer.x * 0.17 * followStrength, 6.5, delta)
  animateRotation(parts.head, 'x', -pointer.y * 0.10 * followStrength + headNod + (state === 'sleeping' ? 0.09 : 0), 6.5, delta)
  animateRotation(parts.head, 'z', pose.headTilt + (state === 'thinking' ? Math.sin(time * 1.7) * 0.012 * motion : 0), 5.5, delta)
  ...
  animateRotation(parts.leftHand, 'z', state === 'happy' ? Math.sin(time * 5.6 + 0.6) * 0.12 * motion : 0, 8, delta)
  ...
  const blink = blinkPhase > 3.2 && blinkPhase < 3.42
    ? Math.sin((blinkPhase - 3.2) / 0.22 * Math.PI) * motion : 0
  const winkPhase = time % 13.1
  const wink = state === 'looking' && winkPhase > 7.6 && winkPhase < 7.95
    ? Math.sin((winkPhase - 7.6) / 0.35 * Math.PI) * motion : 0
  const asleep = state === 'sleeping'
  parts.leftEye.scale.y = smooth(parts.leftEye.scale.y, asleep ? 0.11 : 1.12 * (1 - blink * 0.94), 35, delta)
  parts.rightEye.scale.y = smooth(parts.rightEye.scale.y, asleep ? 0.11 : 1.12 * (1 - Math.max(blink, wink) * 0.94), 35, delta)
  ...
  const pulse = (parts.coreHeart.userData.baseScale ?? 1) * (1 + Math.sin(time * 2.4) * 0.022 * motion)
  parts.coreHeart.scale.setScalar(smooth(parts.coreHeart.scale.x, pulse, 7.5, delta))
  parts.coreMaterial.emissiveIntensity = smooth(parts.coreMaterial.emissiveIntensity, pose.coreIntensity, 7, delta)
  parts.coreLight.intensity = smooth(parts.coreLight.intensity, pose.coreIntensity * 0.16, 7, delta)
  parts.coreHalo.material.opacity = smooth(parts.coreHalo.material.opacity,
    (asleep ? 0.25 : 0.46) + Math.sin(time * 2.4) * 0.07 * motion, 7, delta)
}
