...
function createArm(material: THREE.Material, side: -1 | 1) {
  ...
  for (let row = 0; row <= segments; row += 1) {
    ...
    for (let column = 0; column <= radialSegments; column += 1) {
      ...
      positions.push(
        centerX + Math.cos(angle) * point.x,
        point.y,
...
function createLeafGeometry(side: -1 | 1) {
  ...
  for (let index = 0; index < positions.count; index += 1) {
    const fraction = THREE.MathUtils.clamp(
      (positions.getY(index) + 0.15) / 0.40,
      0,
      1,
    )
...
export function createNexoModel() {
  const pearlMaterial = new THREE.MeshPhysicalMaterial({
    ...
    sheen: 0.32,
    sheenColor: new THREE.Color('#d9eaff'),
  })
  ...
  const torsoProfile = new THREE.CatmullRomCurve3([
    new THREE.Vector3(0, -1.34, 0),
    new THREE.Vector3(0.62, -1.33, 0),
    new THREE.Vector3(1.02, -1.22, 0),
    new THREE.Vector3(1.29, -0.88, 0),
    new THREE.Vector3(1.37, -0.42, 0),
    new THREE.Vector3(1.30, 0.12, 0),
    new THREE.Vector3(1.13, 0.60, 0),
    new THREE.Vector3(0.91, 0.96, 0),
    new THREE.Vector3(0.59, 1.14, 0),
    new THREE.Vector3(0, 1.19, 0),
  ], false, 'centripetal')

  const profilePoints = torsoProfile
    .getPoints(100)
  ...
  const body = enableShadow(
    new THREE.Mesh(bodyGeometry, pearlMaterial),
  )
  ...
  for (const side of [-1, 1] as const) {
    addCurve(
      root,
      [
        new THREE.Vector3(side * 0.83, 0.93, 0.19),
        new THREE.Vector3(side * 1.12, 0.47, 0.36),
        new THREE.Vector3(side * 1.29, -0.03, 0.40),
        new THREE.Vector3(side * 1.29, -0.53, 0.35),
  ...
  const seamPoints = profilePoints
    .filter((point) => point.y > -1.31 && point.y < 0.1)
    .map(
      (point) =>
        new THREE.Vector3(
          0,
  ...
  for (let index = 0; index < headPositions.count; index += 1) {
    ...
    headPositions.setXYZ(
      ...
      Math.sign(y) * Math.pow(Math.abs(y), y < 0 ? 0.63 : 0.96)
        * (y < 0 ? 0.59 : 0.79),
  ...
  const headShell = enableShadow(
    new THREE.Mesh(headGeometry, pearlMaterial),
  )

  head.add(headShell)

  const visorRim = new THREE.Mesh(
    createVisorGeometry(1.805, 0.665, 0.012),
  ...
  const visor = new THREE.Mesh(
    createVisorGeometry(1.77, 0.63, 0.034),
    visorMaterial,
  )
  ...
  addCurve(
    head,
    [
      new THREE.Vector3(-0.70, 0.18, 0.692),
      new THREE.Vector3(-0.43, 0.257, 0.764),
      new THREE.Vector3(-0.05, 0.26, 0.797),
      new THREE.Vector3(0.35, 0.235, 0.777),
    ],
    0.012,
    reflectionMaterial,
  )
  ...
  const eyeBridge = new THREE.Mesh(
  ...
  const leftArmParts = createArm(
    pearlMaterial,
    -1,
  )

  const rightArmParts = createArm(
    pearlMaterial,
    1,
  )
  ...
  root.add(
    leftArmParts.shoulder,
    rightArmParts.shoulder,
  )
  ...
  const chestSensor = new THREE.Mesh(
    new THREE.TorusGeometry(
      0.135,
      0.009,
      10,
  ...
  return {
    ...
    leftShoulder: leftArmParts.shoulder,
    leftForearm: leftArmParts.forearm,
    leftHand: leftArmParts.hand,

    rightShoulder: rightArmParts.shoulder,
    rightForearm: rightArmParts.forearm,
    rightHand: rightArmParts.hand,

    coreGroup: core.group,
    coreHeart: core.heart,
    coreHalo: core.halo,
