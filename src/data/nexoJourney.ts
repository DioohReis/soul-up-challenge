...
export const questCatalog: JourneyQuest[] = ecologicalProblems.flatMap((problem) =>
...
export function getJourneyAchievements(completedIds: readonly string[]): JourneyAchievement[] {
  const completed = questCatalog.filter((quest) => completedIds.includes(quest.id))
  return [
    { id: 'first-step', title: 'Primeiro passo', description: 'Conclua sua primeira missão.', icon: 'star', unlocked: completed.length > 0 },
    { id: 'green-habit', title: 'Em evolução', description: 'Conclua cinco missões.', icon: 'leaf', unlocked: completed.length >= 5 },
    { id: 'recycling', title: 'Novo ciclo', description: 'Conclua uma missão de resíduos.', icon: 'recycle', unlocked: completed.some((quest) => quest.problemId === 'residuos') },
    { id: 'water', title: 'Cada gota conta', description: 'Conclua uma missão de água.', icon: 'drop', unlocked: completed.some((quest) => quest.problemId === 'agua') },
  ]
}
...
export function getJourneyReply(input: string, context: ReplyContext): string {
  ...
  const { userName, points, completedIds, currentProblem, activeQuest, recommendedQuest } = context
  const count = questCatalog.filter((quest) => completedIds.includes(quest.id)).length
  ...
  if (/\b(desanimad[oa]|cansad[oa]|triste|ansios[oa]|sem energia|sem motivacao|dificil|nao consigo|incentivo|motive|motivacao|inspire|inspiracao)\b/.test(text)) {
  ...
  if (/\b(conquista|conquistas|recompensa|recompensas|premio|premios|desconto|trocar)\b/.test(text)) {
    const unlocked = getJourneyAchievements(completedIds).filter((achievement) => achievement.unlocked)
  ...
  if (/\b(missao|missoes|comecar|comeco|sugestao|sugira|recomenda|ajuda|objetivo|o que fazer)\b/.test(text)) {
    return activeQuest
  ...
  if (/\b(oi|ola|bom dia|boa tarde|boa noite|tudo bem|quem e voce|seu nome)\b/.test(text)) {
  ...
  const topics: Array<[string, RegExp]> = [
    ...
    ['biodiversidade', /\b(biodiversidade|planta|plantas|natureza|arvore|arvores)\b/],
    ['poluicao', /\b(poluicao|descartavel|descartaveis|fumaca)\b/],
  ]
  const topic = topics.find(([, pattern]) => pattern.test(text))
  if (topic || /\b(tema|ambiente|impacto|explique|explica|sustentabilidade)\b/.test(text)) {
    const problem = ecologicalProblems.find((item) => item.id === topic?.[0]) ?? currentProblem
